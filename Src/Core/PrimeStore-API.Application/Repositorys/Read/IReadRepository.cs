...
namespace PrimeStore_API.Application.Repositorys.Read
{
    public interface IReadRepository<T> where T : BaseEntity
    {
        ...
        Task<IEnumerable<T>> GetSqlQuery(string tableName, string[]? columns, string? where, bool tracking = true);
        Task<IEnumerable<T>> GetSqlQuery(string query, bool tracking = true);
